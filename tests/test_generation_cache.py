...
def test_cache_roundtrip_and_idempotent_set(tmp_path) -> None:
    ...
    key = cache_key("hash", "L2", "", "", "deepseek-chat", "v0.6.0")
    assert cache.get(key) is None
    cache.set(key, "hash", "L2", "第一版", model="deepseek-chat", prompt_version="v0.6.0")
    cache.set(key, "hash", "L2", "第二版不应覆盖", model="deepseek-chat", prompt_version="v0.6.0")
