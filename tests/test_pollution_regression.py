...
def test_cold_config_wealth_mission_are_honestly_empty(cold_config_chart) -> None:
    for map_type in ("wealth", "mission"):
        package = build_interpretation_map(cold_config_chart, map_type=map_type)
        items = [item for section in package.sections for item in section.items]
        assert items == [], f"{map_type} 不应有硬凑的条目"
        assert any("宁可先空着" in section.intro for section in package.sections)
