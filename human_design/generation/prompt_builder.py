...
1. 你只能使用【本次图表事实】区块里明确给出的内容。区块里没有的闸门号、
   ...
   不得把三个或更多独立闸门另起名称，拼成“能力轴、回路、系统、模块或组合”。
   已定义通道本来就稳定存在；策略、邀请和认可只影响进入事情及表达时机，不能说它们会激活、启动或创造通道。
...
_MAP_TASKS = {
    ...
    "channels": "逐条讲清当前盘面真实接通的通道在生活中会形成什么稳定能力，再综合这些线路如何互相配合、容易怎样误用，以及怎样通过真实作品和关系场景把能力用成熟。",
