...
这是人类图产品的技能入口。当前仓库已经不是单纯的排盘脚本，而是一个 LLM 原生产品层：排盘、阅读对象、会话产品包都在仓库内生成。
...
- Runtime 提示：`runtimes/`
- 知识卡目录：`references/`（当前已包含 types / authorities / profiles / centers / definitions，以及第一批 gates / channels）
- 脚本目录：`scripts/`
- 测试目录：`tests/`
