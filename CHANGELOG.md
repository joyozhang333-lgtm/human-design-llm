...
- 将 API 中的资料来源绝对路径统一转换为仓库内相对引用，避免暴露服务器目录结构
- 为排盘、报告和解读地图增加公共响应脱敏回归测试

## 0.7.0 - 2026-08-30

- 全面重做结果页视觉：采用中性白底、系统字体、细分隔线和克制留白，删除深色编辑风、卡片墙、悬浮按钮与装饰性标签
- 报告改为可连续阅读的单栏文章，章节一次完整呈现，桌面端提供简洁目录，移动端保留自然滚动
- 清理“基本配置、全盘先读、当前章节、说人话的解读”等产品内部语言，标题直接回答用户正在关心的问题
- 把主题追问重构为独立咨询工作区；点击报告问题会携带当前条目进入对话，桌面端并排阅读，移动端切换为完整咨询页
- DeepSeek 对话改为每轮只推进一个判断、3-5 个短段落和一个具体追问；本地回退也不再机械复述报告字段
- 加强用户可见文本净化，拦截 prompt、chart facts、地图上下文、知识原子、校验器等内部内容
- 更新身体、通道、财富、天赋、关系与使命报告的章节标题和导语，减少重复术语，优先呈现现实场景、卡点与行动方法
- 新增前端呈现回归测试；干净提交中的 `227` 个 pytest、前端生产构建、官方 npm 生产依赖审计与 5 份匿名盘面内容审计通过

## 0.6.2 - 2026-08-27

- 修复 GitHub Release 的 `SHA256SUMS` 路径，下载三个发布文件到同一目录后可直接执行标准校验
- Release 工作流在上传前自行执行 `sha256sum --check`，避免再次发布不可用的校验清单

## 0.6.1 - 2026-08-27

- 从公开的主题报告 API 响应中移除服务端 `prompt_pack` 与 `system_prompt`，彻底阻断内部提示词通过开发者接口泄露
- 保留结构化报告、知识原子与来源引用，Agent 和前端调用契约不受影响
- 增加公共接口回归断言，确保后续版本不会重新暴露提示词

## 0.6.0 - 2026-08-27

- 结果页完成结构级改版：BodyGraph 置顶，图后只保留六项核心配置、三条全盘结论、通道解读、五份主题报告与连续咨询
- 从结果页顶层彻底移除已定义中心、开放中心和闸门目录；中心事实只在身体等相关报告中按需解释
- 报告改为安静的单栏阅读流，一次聚焦一章，详细诊断按需展开，避免卡片墙和一次性长文堆叠
- 新增独立通道报告，覆盖当前盘面的每条真实通道，并解释多条能力线路如何组合、误用和成熟
- 升级身体、天赋、财富、关系、使命报告的全盘联动和差异化内容审计，匿名回归样本达到内部内容门槛
- 点击报告问题会携带盘面与章节上下文进入 DeepSeek 连续咨询，不再机械复制报告
- DeepSeek 增加前后端双重同意门槛；未明确同意时不调用外部模型，自动主报告固定走本地确定性引擎
- 增加前后端版本一致性检查，旧标签页最多自动刷新一次并提供明确更新入口
- 新增标准 `skills/human-design/` 包和本地执行适配器，可安装到 Codex、Claude Code、DeepSeek Harness、Hugging Face / OpenClaw 与腾讯 WorkBuddy
- 新增 `.agents`、`.claude`、`.dsh`、`.codebuddy` 项目发现入口、跨宿主安装器，GitHub Release 同时发布 Python wheel、确定性 WorkBuddy ZIP 和 SHA-256 清单
- CI 增加 Agent 路径、Skill 元数据、本地执行、密钥排除与上传包回归
- Agent CLI 拒绝缺失时区的正式排盘，地点联网解析必须显式同意；发布包排除 `.env*`、私钥和凭据文件
- 将专业资料库作为 Python package data 发布，确保 Agent 在源码仓库外安装后仍能生成完整报告
