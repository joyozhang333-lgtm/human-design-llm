...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare two Human Design charts for relationship analysis."
    )
...
def main() -> int:
    args = parse_args()
    try:
        left = normalize_birth_input(
            args.left_birth_time,
            timezone_name=args.left_timezone_name,
        )
        right = normalize_birth_input(
            args.right_birth_time,
            timezone_name=args.right_timezone_name,
        )
    except InputNormalizationError as exc:
        print(f"input error: {exc}", file=sys.stderr)
        return 2

    result = compare_relationship(
        left,
        right,
